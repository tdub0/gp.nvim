...
function M.check()
  ...
  if not ok then
    ...
    ---@diagnostic disable-next-line: undefined-field
    local api_endpoint = gp.config.openai_api_endpoint

    if type(api_key) == "table" then
      vim.health.error(
        "require('gp').setup({openai_api_endpoint: ???}) is still an unresolved command: " .. vim.inspect(api_endpoint)
      )
    elseif api_endpoint and string.match(api_endpoint, "%S") then
      vim.health.ok("config.openai_api_endpoint is set")
    else
      vim.health.error("require('gp').setup({openai_api_endpoint: ???}) is not set: " .. vim.inspect(api_endpoint))
    end
  end
